...
description: |
  ...
  Applies a verified viral-intro formula (reverse-engineered frame-by-frame from a top true-crime-style
  interview channel's 10 most-viewed episodes): 4 VO patterns (A/B/C/D), 7-beat structure, kinetic
  typography, Vacuum Drop, Tonal Whiplash Bridge. REJECTS true-crime aesthetic, persecution/mortality
  framing, CIA/FBI authority stacks (brand-drift) for B2B shows.

  Includes 3 mandatory pre-check gates (identity / speaker / transcript-claim verification), voice &
  taste filters, a per-beat rubric self-scorer with iteration loop, and an incident log of codified
  failure modes from real pilot runs.

  Output: editor one-pager.pdf (landscape A4, 3-col variations) + master pack.pdf + assembly-sheet.csv +
  vo-drafts.md + fit-check.md + rubric scorecard. Synced to the media store + tracker + notify channel.
type: skill
status: active
created: 2026-04-21
updated: 2026-06-11
