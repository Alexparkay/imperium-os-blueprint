...
Load `.claude/skills/wispr-corrections/SKILL.md`. Apply patterns in order:
...
<new_patterns_caught>
- [any new dictation errors not in the dict - append these to wispr-corrections/SKILL.md]
</new_patterns_caught>
```
...
When you detect a dictation error pattern that isn't in `wispr-corrections/SKILL.md`:

1. Fix it in the current parse (Step 1)
2. Append it to Section 9 of `wispr-corrections/SKILL.md` with timestamp + context
...
6. **Self-update the dict.** Every new dictation pattern caught gets appended to `wispr-corrections/SKILL.md` Section 9.
